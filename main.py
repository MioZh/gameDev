...
def reset_game():
    global grid, start_row, start_col, end_row, end_col, coins, game_state, player_pos, timer, show_notification
    grid = create_grid()
...
while not done:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            reset_game()
            game_state = "playing"
            timer = 1800  # Начинаем таймер при нажатии на кнопку "Старт"
        elif event.type == pygame.KEYDOWN and game_state == "playing" and not show_notification:
            if event.key == pygame.K_w and player_pos[0] > 0 and grid[player_pos[0] - 1][player_pos[1]] != 1:
    ...
    if game_state == "menu":
        ...
        pygame.draw.rect(screen, WHITE, [(WINDOW_SIZE[0] - 100) / 2, (WINDOW_SIZE[1] - 50) / 2, 100, 50])
        font = pygame.font.Font(None, 36)
        text = font.render("Старт", True, BLACK)
        screen.blit(text, [(WINDOW_SIZE[0] - 100) / 2 + 20, (WINDOW_SIZE[1] - 50) / 2 + 15])
        ...
        screen.blit(text, [(WINDOW_SIZE[0] - 200) / 2, (WINDOW_SIZE[1] - 100) / 2])

        # Отрисовка кнопки "Вернуться в меню"
        pygame.draw.rect(screen, WHITE, [(WINDOW_SIZE[0] - 240) / 2, (WINDOW_SIZE[1] + 100) / 2, 225, 50])
        font = pygame.font.Font(None, 36)
        text = font.render("Вернуться в меню", True, BLACK)
        screen.blit(text, [(WINDOW_SIZE[0] - 230) / 2 + 2, (WINDOW_SIZE[1] + 100) / 2 + 15])
    ...
    if game_state == "finished":
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if (WINDOW_SIZE[0] - 200) / 2 <= mouse_pos[0] <= (WINDOW_SIZE[0] - 200) / 2 + 200 and \
               (WINDOW_SIZE[1] + 100) / 2 <= mouse_pos[1] <= (WINDOW_SIZE[1] + 100) / 2 + 50:
                game_state = "menu"
                # Сброс игры до начального состояния, если требуется
                player_pos = [start_row, start_col]
                timer = 1800
                # coins = []
                show_notification = False
    ...
    if show_notification:
        ...
        if pygame.mouse.get_pressed()[0]:
            ...
            if WINDOW_SIZE[0] - 150 <= mouse_pos[0] <= WINDOW_SIZE[0] - 10 and 210 <= mouse_pos[1] <= 240:
                ...
                player_movement_allowed = True  # Возобновляем движение игрока
    # Обновление экрана
    pygame.display.flip()
